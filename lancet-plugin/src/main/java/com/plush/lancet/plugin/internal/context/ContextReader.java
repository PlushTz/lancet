...
import com.plush.lancet.plugin.internal.preprocess.ParseFailureException;
...
public class ContextReader {
    ...
    private ExecutorService service = new ThreadPoolExecutor(Runtime.getRuntime().availableProcessors(), Runtime.getRuntime().availableProcessors(), 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<>(), (r, executor) -> {
    ...
    public void accept(boolean incremental, ClassFetcher fetcher) throws IOException, InterruptedException {
        ...
        // get all jars
        Collection<JarInput> jars = !incremental ? context.getAllJars() : ImmutableList.<JarInput>builder().addAll(context.getAddedJars()).addAll(context.getRemovedJars()).addAll(changedToDeleteAndAdd()).build();
        // accept the jar in thread pool
        List<Future<Void>> tasks = Stream.concat(jars.stream(), context.getAllDirs().stream()).map(q -> new QualifiedContentTask(q, fetcher)).map(t -> service.submit(t)).collect(Collectors.toList());
    ...
    private Collection<? extends JarInput> changedToDeleteAndAdd() {
        ...
        context.getChangedJars().stream().peek(c -> jarInputs.add(new StatusOverrideJarInput(context, c, Status.REMOVED))).peek(c -> jarInputs.add(new StatusOverrideJarInput(context, c, Status.ADDED)));
        return jarInputs;
    }
