...
import com.plush.lancet.plugin.internal.GlobalContext;
import com.plush.lancet.plugin.internal.LocalCache;
import com.plush.lancet.plugin.internal.TransformContext;
import com.plush.lancet.plugin.internal.TransformProcessor;
import com.plush.lancet.plugin.internal.context.ContextReader;
import com.plush.lancet.plugin.internal.preprocess.PreClassAnalysis;
import com.plush.lancet.weaver.MetaParser;
import com.plush.lancet.weaver.Weaver;
import com.plush.lancet.weaver.internal.AsmWeaver;
import com.plush.lancet.weaver.internal.entity.TransformInfo;
import com.plush.lancet.weaver.internal.log.Log;
import com.plush.lancet.weaver.internal.log.impl.FileLoggerImpl;
import com.plush.lancet.weaver.internal.parser.AsmMetaParser;
...
public class LancetTransform extends Transform {
    ...
    public Collection<SecondaryFile> getSecondaryFiles() {
        return cache.hookClassesInDir().stream().map(File::new).map(SecondaryFile::nonIncremental).collect(Collectors.toList());
    }
    ...
    private AsmMetaParser createParser(TransformContext context) {
        URL[] urls = Stream.concat(context.getAllJars().stream(), context.getAllDirs().stream()).map(QualifiedContent::getFile).map(File::toURI).map(u -> {
            try {
                return u.toURL();
