    ...
    public static class FlowNode {
        ...
        public boolean checkNormally(Node n, Graph graph) {
            ...
            for (FlowNode c : children) {
                ...
                if (child == null && !c.allModified(graph) || child != null && !c.checkNormally(child, graph)) {
                    return false;
                }
            }
            if (!map.values().stream().allMatch(FlowNode::neatInheritance)) {
                return false;
            }
            n.checkPass[1] = true;
            return true;
        ...
        private boolean allModified(Graph graph) {
            Node t;
            return ((t = graph.get(className)) == null || t.status != Status.NOTCHANGED) && children.stream().allMatch(c -> c.allModified(graph));
        }


        private static boolean neatInheritance(Node n) {
            return n.status != Status.NOTCHANGED && directChildrenOf(n).values().stream().allMatch(FlowNode::neatInheritance);
        }

        private static Map<String, Node> directChildrenOf(Node n) {
            InterfaceNode t;
            return n instanceof ClassNode ? ((ClassNode) n).children.stream().collect(Collectors.toMap(v -> v.entity.name, v -> v)) : Stream.concat((t = (InterfaceNode) n).children.stream(), t.implementedClasses.stream()).collect(Collectors.toMap(v -> v.entity.name, v -> v));
        }
    }
}
