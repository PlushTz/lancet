...
import com.plush.lancet.weaver.internal.asm.classvisitor.methodvisitor.AutoUnboxMethodVisitor;
...
public class MethodChain {
    ...
    private void head(int access, int opcode, String owner, String name, String desc) {
        this.head = Invoker.forMethod(new MethodInsnNode(opcode, owner, name, desc, opcode == Opcodes.INVOKEINTERFACE), !hasPermission(access, owner), className);
    }
    ...
    private void dealField(int opcode, String name, MethodVisitor mv) {
        ...
        invokerMap.computeIfAbsent(opcode + " " + name, k -> {
    ...
    private void initFields() {
        if (fieldMap == null) {
            this.fieldMap = graph.get(className).entity.fields.stream().collect(Collectors.toMap(f -> f.name, f -> f));
        }
    }
    ...
    public static class Invoker implements Opcodes {
        ...
        private static String staticDesc(String className, MethodInsnNode mn, FieldInsnNode fn) {
            String desc = mn != null ? mn.desc : (fn.getOpcode() == PUTFIELD ? '(' + fn.desc + ")V" : "()" + fn.desc);
        ...
        public void createIfNeed(ClassVisitor cv, Bitset bitset, String[] exceptions) {
            ...
            if (needCreate) {
                ...
                createMethod(ACC_STATIC, staticDesc, mn == null ? fn : mn).accept(mv);
            }
        }
            ...
            if (mn != null) {
                ...
                if (fn.getOpcode() == PUTFIELD) { // unbox
                    ...
                    if (PrimitiveUtil.isPrimitive(fn.desc)) {
                        ...
                        mv.visitMethodInsn(INVOKEVIRTUAL, PrimitiveUtil.virtualType(owner), PrimitiveUtil.unboxMethod(owner), "()" + fn.desc, false);
                    }
                    invoke(mv);
                } else { // box
                    mv.visitVarInsn(ALOAD, 0);
                    ...
                    if (PrimitiveUtil.isPrimitive(fn.desc)) {
                        String owner = PrimitiveUtil.box(fn.desc);
                        mv.visitMethodInsn(INVOKESTATIC, owner, "valueOf", "(" + fn.desc + ")L" + owner + ";", false);
